...
We can also modify an existing variable by writing an expression that computes an new value using it and reassigning the result to the same name. 
...
That said, you should also feel free to include explanatory text *outide* of code chunks if you are working in an `Rmarkdown` document like this one. Just be aware that the `#` symbol has a very different meaning in this context: it indicates header text, not a comment.
...
Variables in R can store different kinds of data, such as numbers, character strings (text), or boolean (`TRUE`/`FALSE`) values, as well as more complicated data structures.
To keep track of this, each variable has not just a value, but also a  **type**. 
Most of the time, these will be set automatically, as in the example below where we define a variable with a number.
...
Occasionally, we will want to change the type of a variable, which we call _coercion_.
Let's _coerce_ our `x` variable to a character type. 
...
See that the value now has quotes around it? It's now a character and will behave as such
...
A vector in R is a sequence of values that all have the same type. 
...
Imagine defining a variable called `c`: This will work, but it will lead to a lot of unintended bugs, so its best to avoid this. 
...
_Data frames are one of the most fundamental units of data analysis in R._ 
They are tables which consist of rows and columns, much like a _spreadsheet_. 
Each column is a variable which behaves as a _vector_ (all data is the same type), and each row is an observation. 
...
We might want to explore individual columns of the data frame in more depth. 
